...
if menu == "Accueil":
    ...
    # ---------------------- Température et Humidité par année ----------------------
    if "Date" in df.columns and "Temperature (C)" in df.columns and "Humidity" in df.columns:
        st.subheader("🌡️ Température et Humidité par année")

        # Conversion sécurisée en datetime
        df['Date'] = pd.to_datetime(df['Date'], errors='coerce', dayfirst=True)

        # Supprimer lignes sans date valide
        df = df[df['Date'].notna()]
        ...
        if df.empty:
            ...
            # ---------------------- Nombre de mesures et moyenne température ----------------------
            nb_mesures = len(df)
...
# ------------------ Page Régression Linéaire ------------------
    ...
    st.subheader("Visualisation des paramètres et de la prédiction")
